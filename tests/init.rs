//! Integration tests for `autograder::init::init` -- everything here only
...
fn init_produces_a_loadable_library_package() {
    ...
    let outcome = autograder::init::init(dir.path(), "hw3", AssignmentKind::Library).unwrap();
...
fn init_produces_a_loadable_binary_package_with_a_harness_dir() {
    let dir = tempfile::tempdir().unwrap();
    let outcome = autograder::init::init(dir.path(), "wc", AssignmentKind::Binary).unwrap();
...
fn init_rejects_an_id_that_would_be_an_invalid_package_name() {
    let dir = tempfile::tempdir().unwrap();
    let err =
        autograder::init::init(dir.path(), "3-bad start", AssignmentKind::Library).unwrap_err();
...
fn init_refuses_to_write_into_a_nonempty_directory() {
    ...
    let err = autograder::init::init(dir.path(), "hw3", AssignmentKind::Library).unwrap_err();
    assert!(matches!(err, Error::InvalidSpec(_)));
}
