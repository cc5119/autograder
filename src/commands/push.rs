...
pub fn run(assignment: &Path) -> Result<()> {
    let vcs = Vcs::detect();
    if vcs.already_has_a_local_bookmark(assignment) {
    ...
    let mode = PublishMode::prompt("Push as")?;
    let year = Zoned::now().year();
    let repo_name = format!("{year}-{semester:02}-{id}-{}", mode.repo_suffix());
    ...
    vcs.init_and_commit(assignment)?;
    ...
    vcs.add_remote_and_push(assignment, &remote_url)?;
