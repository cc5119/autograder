//! `autograder register`: shows what registering every roster student
...
use crate::register::{Outcome, RegisterPlan, Status, plan_register, register_batch};
use crate::render;
...
pub fn run(roster: &Path, org: &str, team: &str, yes: bool) -> Result<()> {
    ...
    let plan = plan_register(org, team, &roster)?;

    print!("{}", render_plan(&plan));
    if !confirm(plan.to_register(), yes)? {
        tracing::info!("nobody registered");
        return Ok(());
    }

    register_batch(&plan, &report)?;
    Ok(())
}
...
fn report(github_user: &GithubUser, outcome: &Outcome) {
    match outcome {
        ...
        Outcome::Failed { message } => tracing::warn!(%github_user, message, "register failed"),
    }
}

/// The full manifest, in roster order: every student and what they'd get.
/// No collapsing -- who is already registered and who is a typo is exactly
/// what the instructor is being asked to confirm.
fn render_plan(plan: &RegisterPlan) -> String {
    let mut s = String::new();
    let _ = writeln!(
        s,
        "\nRegistering {} students in {}",
        plan.rows.len(),
        style(&plan.org).bold()
    );
    let _ = writeln!(
    ...
    let _ = writeln!(
        s,
        "  {} on the roster: {registering} to register, {} already on the team",
        plan.rows.len(),
        on_team
    );
    if unknown > 0 {
...
fn count(plan: &RegisterPlan, f: impl Fn(&Status) -> bool) -> usize {
    plan.rows.iter().filter(|row| f(&row.status)).count()
}

/// Refuses rather than assumes when there's no one to ask: registering
/// emails real people, so an unattended run has to say `--yes` out loud.
fn confirm(to_register: usize, yes: bool) -> Result<bool> {
    if yes {
        return Ok(true);
    }
    if to_register == 0 {
        println!("Nobody to register.");
        return Ok(false);
    }
    if !Term::stdout().is_term() {
        return Err(Error::Other(
            "refusing to register without confirmation: not running on a terminal, pass --yes"
                .to_string(),
        ));
    }

    Confirm::new()
        .with_prompt(format!("Register {to_register} students?"))
        .default(false)
        .interact()
